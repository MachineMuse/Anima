  ...
  @SubscribeEvent def gatherData(implicit event: GatherDataEvent): Unit = {
    provideArbitrarySerializer[ElementalBehaviour]("crop_elements")(
      modLoc("beets") -> ElementalBehaviour(Blocks.BEETROOTS,
        infuses=List(SoilElement.Darkness),
        consumes = List(SoilElement.Earth),
        likes = List(SoilElement.Water),
        dislikes = List(SoilElement.Air)),
      ...
      modLoc("wheat") -> ElementalBehaviour(Blocks.WHEAT,
        infuses=List(),
        consumes = List(SoilElement.Air),
        likes = List(),
        dislikes = List()),

      modLoc("cactus") -> ElementalBehaviour(Blocks.CACTUS,
        infuses=List(SoilElement.Fire),
        consumes = List(SoilElement.Earth),
        likes = List(SoilElement.Water),
        dislikes = List(SoilElement.Air)),

      modLoc("sugar_cane") -> ElementalBehaviour(Blocks.SUGAR_CANE,
        infuses=List(SoilElement.Air),
        consumes = List(SoilElement.Water),
        likes = List(SoilElement.Water),
        dislikes = List()),
      ...
      modLoc("radish") -> ElementalBehaviour(Radish.BLOCK.get,
        infuses=List(SoilElement.Fire),
        consumes = List(SoilElement.Air),
        likes = List(),
        dislikes = List()),

      modLoc("brown_mushroom") -> ElementalBehaviour(Blocks.BROWN_MUSHROOM,
        infuses=List(SoilElement.Earth, SoilElement.Air),
        consumes = List(SoilElement.Darkness),
        likes = List(),
        dislikes = List()),

      modLoc("red_mushroom") -> ElementalBehaviour(Blocks.RED_MUSHROOM,
        infuses=List(SoilElement.Earth, SoilElement.Air),
        consumes = List(SoilElement.Darkness),
        likes = List(),
        dislikes = List()),
